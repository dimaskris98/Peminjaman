...
class login_model extends CI_Model{
 ...
 $query = $this->db->query("SELECT * FROM (SELECT gedung.NIP,peminjaman.ID_Peminjam,peminjaman.Tanggal_pinjam,gedung.Nama_Gedung,peminjaman.Lama_pinjam,peminjaman.Keperluan,pengguna.NIM,pengguna.Nama_Pengguna,peminjaman.Status FROM pengguna,peminjaman,gedung where pengguna.NIM = peminjaman.NIM AND peminjaman.ID_Gedung=gedung.ID_Gedung) as a WHERE a.Tanggal_pinjam LIKE '$keyword2-$keyword-%' ");
	...
	function info(){
		...
		$query = $this->db->query ("SELECT * FROM (SELECT peminjaman.Tanggal_pinjam,gedung.Nama_Gedung,peminjaman.Lama_pinjam,peminjaman.Keperluan,pengguna.Nama_Pengguna,peminjaman.Status FROM pengguna,peminjaman,gedung where pengguna.NIM = peminjaman.NIM AND peminjaman.ID_Gedung=gedung.ID_Gedung) AS a where a.Status = 'disetujui'");
